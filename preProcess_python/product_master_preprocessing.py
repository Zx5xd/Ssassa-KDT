...
def extract_product_detail_info(products, cat_name):
    ...
    for pd in products:
        ...
        #                'category_id': item[0].get('id'), 'category_child_id': item[0].get('category_id', -1),
        #                'default_variant': -1, 'visit_count' : 0,
        #                'amount': 0}
        #
        ...
        #     simple_img_id = find_img_path_id(pd.get('simpleImg'), pd.get("productName"),
        #                                      "[ detail Info // Variation ] simple_img")
        #     # detail_img_id = find_img_path_id(pd.get('detailImg'), pd.get("productName"), "[ detail Info ] detail_img")
        #
        #     product = {'name': pd.get("productName"), 'simple_img': simple_img_id,
        ...
        #                'category_id': item[0].get('id'), 'category_child_id': item[0].get('category_id', -1),
        #                'default_variant': -2, 'visit_count' : 0,
        #                'amount': 0}
        #
        #     print(f"[ Variation Product ] pd : {product}")
        #
        #     product_list.append(product)

        simple_img_id = find_img_path_id(pd.get('simpleImg'), pd.get("productName"), "[ simple_img ]")

        detail_img = pd.get("detailImg")
        detail_img_id = find_img_path_id(detail_img, pd.get("productName"), "[ detail_img ]") if detail_img else 0

        # detail이 존재하는 경우만 JSON 변환
        detail_json = None
        if isinstance(pd.get("detail"), dict):
            detail_json = json.dumps(pd.get("detail"), ensure_ascii=False)

        product = {
            'name': pd.get("productName"),
            'simple_img': simple_img_id,
            'detail_img': detail_img_id,
            'price': pd.get("price"),
            'detail': detail_json,
            'reg': pd.get("reg"),
            'category_id': item[0].get('id'),
            'category_child_id': item[0].get('category_id', -1),
            'default_variant': -1 if detail_json else -2,
            'visit_count': 0,
            'amount': 0
        }
...
def extract_product_variation_info(products):
    ...
    for pd in products:
        ...
        if pd.get('variants'):
            find_id = find_img_path_id(pd.get("simpleImg"), pd.get("productName"),
                                       "[ variation Info ] simpleImg find_id")
            item = [item for item in master_items if
                    item['name'] == pd.get('productName') and item['simple_img'] == find_id]
            ...
            for data in pd.get("variants"):
                ...
                detail_img_id = find_img_path_id(data['detailImg'], pd.get("productName"),
                                                 "[ variation Info ]  detail_img")

                variant = {'master_id': item[0].get("id"), 'price': data['price'],
...
def export_variant_info_to_update():
    variant_items = mysql_crud.crud_select("product_variant").to_dict(orient="records")
    # master_items = mysql_crud.crud_select("product_master").to_dict(orient="records")

    # master_id 기준으로 variant 묶기
    variant_dict = defaultdict(list)
    for v in variant_items:
        if variant_dict[v['master_id']]:
            continue
        variant_dict[v['master_id']].append(v['id'])
